...
app.get("/", (req, res) => {
  ...
  res.send("Rotas: \n" + "/usuarios -> para listar todos os usuarios \n" + "/usuarios/:id -> para listar um usuario especifico pelo codigo do id");
});
    ...
    client.query(
      ...
      function (err, result) {
        ...
        if (result.rows.length > 0)
          ...
          res.send("Não há usuário com este código: " + req.params.id);
        //console.log(result);
      }
    );
  } catch (error) {
...
app.delete("/usuarios/:id", (req, res) => {
  try {
    ...
    client.query(
      "DELETE FROM Usuarios WHERE id = $1", [id], (err, result) => {
        if (err) {
